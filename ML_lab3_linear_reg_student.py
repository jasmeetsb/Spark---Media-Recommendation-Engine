...
def parsePoint(line):
    ...
    #<FILL IN>
    sm = line.split(',')
    lbl = LabeledPoint(sm[0],[sm[1],sm[2],sm[3],sm[4],sm[5],sm[6],sm[7],sm[8],sm[9],sm[10],sm[11],sm[12]])
    return lbl
...
def squaredError(label, prediction):
    ...
    """
    sqrerror = pow((label - prediction),2)
    return float(sqrerror)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    errormean= labelsAndPreds.map(lambda x: squaredError(x[0],x[1]))
    sqrterror = math.sqrt(errormean.mean())
    #sqrt2 = errormean.mean()
    return sqrterror
...
print exampleRMSE
print labelsAndPreds.collect()[0]
...
def gradientSummand(weights, lp):
    ...
    #lb=LinearRegressionModel.train(lp.features, iterations=10,initialWeights=weights)
    #return lb
